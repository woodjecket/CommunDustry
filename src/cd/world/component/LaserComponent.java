...
public class LaserComponent extends BaseComponent{
    ...
    public boolean isProvideLaserEnergy(Building b, int bx, int by){
        if(provideLaserEnergy){
            ...
            if(!(((b.tile.x == bx) ? 0 : sign(bx - b.tile.x)) == dir.x && ((b.tile.y == by)
            ? 0 : sign(by - b.tile.y)) == dir.y)) return false;
    ...
    private void updateLaserEnergy(Posc b){
        ...
        if(child.block instanceof IComp comp && childLaser.getLaserEnergy() >
        comp.getComp(LaserComponent.class).maxLaserEnergy
        ...
        var outRangeDst = max(attenuateRange, dst(b.tileX(), b.tileY(),
        child.tileX(), child.tileY())) - attenuateRange;
    ...
    public void onEntityDraw(Building b){
        ...
        Draw.z(Layer.power);
        //画笔颜色根据电够不够混合一下
        Draw.color(laserColor1, laserColor2,
        (1f - bLaser.getLaserEnergy() / maxLaserEnergy) * 0.86f + Mathf.absin(3f, 0.1f));
        //画笔不透明度改成设置里的
        Draw.alpha(Renderer.laserOpacity);
        //算一算目前激光宽度（Width），基础加波动
        float w = laserBaseWidth + Mathf.absin(pulseScl, pulseMag);
        //四面开花
        //格子值存在，且（那个方块不是同类结点，格子的xy都和自己的不一样，
        //格子后建造且他连接范围不比自己长，或者他连接范围就比自己短这四个条件满足其一）
        ...
        (bLaser.getLaserChild().id > b.id && laserRange >= node.getLaserRange()) || laserRange > node.getLaserRange())){
            ...
            int dst = Math.max(Math.abs(bLaser.getLaserChild().tile.x - b.tile.x),
            Math.abs(bLaser.getLaserChild().tile.y - b.tile.y));
            ...
            if(dst > 1 + b.block.size / 2){
                ...
                Drawf.laser(laser, laserStart, laserEnd, b.x + poff * b.block.size * point.x,
                b.y + poff * b.block.size * point.y,
                bLaser.getLaserChild().tile.worldx() - poff * point.x,
                bLaser.getLaserChild().tile.worldy() - poff * point.y, w);
            }
        }
        Draw.reset();
    }
    ...
    public void onSetStats(Block b){
        b.stats.add(CDStats.maxLaser, maxLaserEnergy);
        if(laserEnergyOutput != 0) b.stats.add(CDStats.laserOutput, laserEnergyOutput, CDStats.perConsume);
        if(consumeLaserEnergy != 0) b.stats.add(CDStats.laserConsume, consumeLaserEnergy, CDStats.perConsume);
    }
