...
public class DirectLaserProvider extends Block{
    ...
    public DirectLaserProvider(String name){
        super(name);
        allowDiagonal = false;
        drawArrow = true;
        update = true;
    ...
    public void drawPlace(int x, int y, int rotation, boolean valid){
        ...
        for(int j = 1 + offset; j <= range + offset; j++){
            ...
            if(other != null && other.team == Vars.player.team() && other instanceof LaserInterface && ((LaserInterface)other).isAbsorbLaserEnergy(x, y)){
                maxLen = j;
                dest = other;
                break;
            }
    ...
    public class DirectLaserProviderBuild extends Building implements LaserInterface{
        ...
        public void draw(){
            ...
            if(laserChild != null && (!(laserChild.block instanceof DirectLaserProvider node) ||
            (laserChild.tileX() != tileX() && laserChild.tileY() != tileY()) ||
            (laserChild.id > id && range >= node.range) || range > node.range)){
